package datamine.query.functions;


import datamine.query.data.Column;
import datamine.query.data.Row;
import datamine.query.data.Value;
import datamine.query.data.ValueUtils;
import gnu.trove.map.hash.THashMap;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

import java.util.HashMap;
import java.util.List;
import java.util.Map;

public class HistDistinctCount extends AggOperation {

    public static final Logger logger = LoggerFactory.getLogger(HistDistinctCount.class);

    private final int threshold;
    private final Column valColumn;
    private final Map<String, THashMap<String, Long>[]> distinctKeyCountMap = new HashMap<>();

	public HistDistinctCount(Column binColumn, Column valColumn) throws Exception {
		super(binColumn);
		this.valColumn = valColumn;
		this.threshold = 0;
		if (binColumn.getType() != ValueUtils.ListLongDataType) {
			throw new Exception("Cannot apply histogram approach to the data type: "
				+ binColumn.getType());
		}
	}

    public HistDistinctCount(Column binColumn, Column valColumn, int threshold) throws Exception {
        super(binColumn);
        this.valColumn = valColumn;
        this.threshold = threshold;
        if (binColumn.getType() != ValueUtils.ListLongDataType) {
            throw new Exception("Cannot apply histogram approach to the data type: "
                + binColumn.getType());
        }
    }

	// Get the starting point for aggregation: the bin range is fetched and an array of values
    // is created as records. However, only the ends of the array are set for necessity.
	@Override
	public Value apply(Row input) throws Exception {
        String val = valColumn.applyOperation(input).getValue().toString();
        List<Long> ends = (List<Long>) col.applyOperation(input).getValue();
        String[] valStrArr = new String[(int) (ends.get(1) + 1)];
        valStrArr[ends.get(0).intValue()] = valStrArr[ends.get(1).intValue()] = val;
        return new Value(valStrArr, ValueUtils.ArrayStringDataType);
	}

	// Iteratively combine counts from different workers
	@Override
	public Value apply(Value base, Value delta) throws Exception {
		if (base == null) {
			return delta;
        }

        long[] baseArr = (long[]) base.getValue();
        long[] deltaArr = (long[]) delta.getValue();

        if (baseArr.length >= deltaArr.length) {
            for (int i = 0; i < deltaArr.length; ++i) {
                baseArr[i] += deltaArr[i];
            }
            return base;
        } else {
            for (int i = 0; i < baseArr.length; ++i) {
                deltaArr[i] += baseArr[i];
            }
            return delta;
        }
    }

    // Interface for aggregates over nested table
	@Override
	public Value apply(String key, Value base, Value delta) throws Exception {

        String[] deltaArr = (String[]) delta.getValue();

        //1. if base is null, a basic count would be calculated (all 1s)
        if (base == null) {
            THashMap[] valSetArray = new THashMap[deltaArr.length];
            long[] counts = new long[deltaArr.length];
            int numOfVals = 2;
            String val = null;
            for (int i=0; i<valSetArray.length; ++i) {
                if (deltaArr[i] != null) {
                    numOfVals--;
                    val = deltaArr[i];
                }

                if (val != null) {
                    valSetArray[i] = new THashMap<>();
                    valSetArray[i].put(val, 1l);

                    if (1 == threshold) {
                        counts[i] = 1;
                    }
                }

                if (numOfVals <= 0) {
                    break; // may not be necessary as the delta is over.
                }
            }
            distinctKeyCountMap.put(key, valSetArray);
            return new Value(counts, ValueUtils.ArrayLongDataType);
        }

        //2. if the base does exist, the arrays holding the counts and
        // the value sets should be extended to cover additional buckets
        // from the delta
        long[] baseArr = (long[]) base.getValue();
        String[] deltaValArr = (String[]) delta.getValue();
        baseArr = extendLongValArray(baseArr, deltaArr.length);
        THashMap<String, Long>[] valSetArray = extendValSetArray(
            distinctKeyCountMap.get(key), deltaValArr.length);

        //3. merge the delta into the base
        int numOfVals = 2;
        String val = null;
        for (int i=0; i<deltaValArr.length; ++i) {
            if (deltaValArr[i] != null) {
                val = deltaValArr[i];
                numOfVals--;
            }

            if (val != null) {
                THashMap<String, Long> curSet = valSetArray[i];
                if (curSet == null) {
                    curSet = new THashMap<>();
                }


                if (!curSet.contains(val)) {
                    curSet.put(val, 1l);
                } else {
                    long newCount = curSet.get(val) + 1;
                    curSet.put(val, newCount);
                }

                if (curSet.get(val) == threshold) {
                    baseArr[i] ++;
                }

                valSetArray[i] = curSet; // in case it was null
            }

            if (numOfVals <= 0) {
                break;
            }
        }
        distinctKeyCountMap.put(key, valSetArray);
        return new Value(baseArr, ValueUtils.ArrayLongDataType);
	}

    private THashMap<String, Long>[] extendValSetArray(THashMap<String, Long>[] valSetArray,
                                                       int len) {
        if (valSetArray != null && valSetArray.length >= len) {
            return valSetArray;
        } else {
            THashMap<String, Long>[] newValSetArray = new THashMap[len];
            if (valSetArray != null) {
                System.arraycopy(valSetArray, 0, newValSetArray, 0, valSetArray.length);
            }
            return newValSetArray;
        }
    }

    private long[] extendLongValArray(long[] valArray, int len) {
        if (valArray != null && valArray.length >= len) {
            return valArray;
        } else {
            long[] newValSetArray = new long[len];
            if (valArray != null) {
                System.arraycopy(valArray, 0, newValSetArray, 0, valArray.length);
            }
            return newValSetArray;
        }
    }

	@Override
	public String getID() {
		return "hist_count(" + col.getID() + " distinct " + valColumn.getID() +
            ", min_freq " + threshold + ")";
	}

	@Override
	public int getResultValueType() {
		return ValueUtils.ArrayLongDataType;
	}

    @Override
    public void clear() {
        this.distinctKeyCountMap.clear();
    }
}
